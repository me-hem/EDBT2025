...
incViewQuery = pd.DataFrame(columns=["epoch", "number", "avgBurntFee", "time", "size"])
for blk_num in range(19000000, 19001024):
    block = GetBlock(blk_num)
    if block:
        incViewQuery = IngestData(block, incViewQuery)
incViewQuery.to_csv("incViewQuery.csv", index=False)